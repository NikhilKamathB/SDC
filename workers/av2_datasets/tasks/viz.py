...
from tasks.src.base import AV2Base
from tasks.src.constants import CAMERA_TYPE_NAME
from tasks.src.enum import AV2ForecastingAnalyticsAttributes
from tasks.src.utils import av2_plot_polylines, av2_plot_bbox, write_video
from tasks.src.validators import InternalCameraMountResponse, SensorInformation, ActorInformation, AV2MotionForecastingTimeStep, Vehicle as AV2Vehicle
...
class AV2Forecasting(AV2Base):
    ...
    def __init__(self, *args, **kwargs) -> None:
        ...
        self.av_configuration_path = kwargs.get(
            "av_configuration_path", "./data/config/agroverse/vehicle0.yaml")
    ...
    def _init_visualization(self) -> None:
        ...
        self.output_directory = os.path.join(
            self.output_directory, self.scenario_id)
        ...
        self.scenario = scenario_serialization.load_argoverse_scenario_parquet(
            Path(os.path.join(self.input_directory,
                 self.scenario_id, self.scenario_file))
        )
        self.static_map = ArgoverseStaticMap.from_json(
            Path(os.path.join(self.input_directory,
                 self.scenario_id, self.static_map_file))
        )
        self.av_configuration = self._get_av_configuration()
    
    def _read_yaml(self, yaml_path: str) -> dict:
        """
    ...
    def _bilinear_interpolate(self, patch: np.ndarray, x_frac: float = 1.0, y_frac: float = 1.0) -> np.ndarray:
        """
    ...
    def _compute_total_distance(self, points: np.ndarray) -> Tuple[float, np.ndarray]:
        """
    ...
    def _make_output_directory(self) -> None:
        ...
        """
        logger.info(
            f"{self.__LOG_PREFIX__}: Making output directory if not exists.")
    ...
    def _get_av_configuration(self) -> AV2Vehicle:
        ...
        """
        logger.info(
            f"{self.__LOG_PREFIX__}: Getting AV - Autonomous Vehicle configuration.")
        if os.path.exists(self.av_configuration_path):
            return AV2Vehicle(**self._read_yaml(self.av_configuration_path))
        return AV2Vehicle()
    ...
    def _generate_scenario_video(self, output_filename: str) -> str:
        ...
        logger.info(
            f"{self.__LOG_PREFIX__}: Scenario video generated at: {output_path}")
        return output_path
    ...
    def _get_actor_states(self, track: Track, timestep: int = None) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
        ...
        actor_positions = np.array([list(object_state.position)
                                   for object_state in track.object_states if timestep is not None and object_state.timestep <= timestep])
        actor_headings = np.array(
            [object_state.heading for object_state in track.object_states if timestep is not None and object_state.timestep <= timestep])
        actor_velocities = np.array([list(object_state.velocity)
                                    for object_state in track.object_states if timestep is not None and object_state.timestep <= timestep])
    ...
    def _mount_sensors(self, ax: plt.Axes, actor_bbox: np.ndarray, actor_heading: np.ndarray) -> List[Union[InternalCameraMountResponse]]:
        ...
        if self.av_configuration.sensors is not None and self.av_configuration.sensors.cameras:
            for idx, camera in enumerate(self.av_configuration.sensors.cameras):
                ...
                mount_point = self._bilinear_interpolate(
                    patch=actor_bbox, x_frac=x, y_frac=y)
    ...
    def _get_angle_and_unit_vector(self, point1: np.ndarray, point2: np.ndarray) -> Tuple[float, np.ndarray]:
        ...
        """
        logger.debug(
            f"{self.__LOG_PREFIX__}: Getting angle and unit vector given two points.")
        delta = point2 - point1
    ...
    def _get_line_from_point_and_angle(self, ax: plt.Axes, point: np.ndarray, theta: float) -> np.ndarray:
        ...
        logger.debug(
            f"{self.__LOG_PREFIX__}: Getting line from a given point and angle.")
        ...
        for candidate in candidates:
            _, (x_dash_hat, y_dash_hat) = self._get_angle_and_unit_vector(
                point, candidate)
            if np.isclose(np.dot([x_hat, y_hat], [x_dash_hat, y_dash_hat]), 1.0):
                lines.append(candidate)
        if not lines or len(lines) > 1:
            logger.warning(
                f"{self.__LOG_PREFIX__}: Multiple lines found for the given point and angle. Selecting the first line.")
        # Plot line
        point_2 = lines[0]
        plt.plot(
            [point[0], point_2[0]],
    ...
    def _get_limits(self, ax: plt.Axes) -> List[np.ndarray]:
        ...
        """
        logger.debug(
            f"{self.__LOG_PREFIX__}: Getting coverage extreme points.")
        xmin, xmax = ax.get_xlim()
        ymin, ymax = ax.get_ylim()
        extreme_points = [
            np.array([xmin, ymin]),
    ...
    def _arrange_polygon_bounds(self, polygon_bounds: List[np.ndarray], centroid: np.ndarray = None) -> List[np.ndarray]:
        ...
        """
        logger.debug(
            f"{self.__LOG_PREFIX__}: Arranging polygon bounds in anti-clockwise order.")
        polygon = np.array(polygon_bounds)
        if centroid is None:
            centroid = np.mean(polygon, axis=0)
        angles = np.arctan2(polygon[:, 1] - centroid[1],
    ...
    def _get_polygon_bounds_between_angle(self, points: np.ndarray, pivot_point: np.ndarray, upper_bound_point: np.ndarray, lower_bound_point: np.ndarray) -> List[np.ndarray]:
        ...
        """
        logger.debug(
            f"{self.__LOG_PREFIX__}: Getting polygon bounds between two angles.")
        # Get angular bounds
        u_theta, _ = self._get_angle_and_unit_vector(
            pivot_point, upper_bound_point)
        l_theta, _ = self._get_angle_and_unit_vector(
            pivot_point, lower_bound_point)
    ...
    def _get_sensor_coverage(self, ax: plt.Axes, pivot_point: np.ndarray, upper_bound_point: np.ndarray, lower_bound_point: np.ndarray) -> np.ndarray:
        ...
        polygon_vertex = self._arrange_polygon_bounds(
            self._get_polygon_bounds_between_angle(
                extreme_points, pivot_point, upper_bound_point, lower_bound_point)
        )
    ...
    def _get_actors_in_sensor_coverage(self, sensor_coverage: ShapelyPolygon, timestep: int) -> Tuple[Dict[ShapelyPolygon, ShapelyPolygon], List[str]]:
        ...
        """
        logger.info(
            f"{self.__LOG_PREFIX__}: Getting actors in the sensor coverage.")
        actors = dict()
        actor_ids = []
        for track in self.scenario.tracks:
            actor_positions, actor_headings, _ = self._get_actor_states(
                track, timestep)
    ...
    def _get_actors_extrimities(self, pivot_point: np.ndarray, actors: Dict[ShapelyPolygon, np.ndarray]) -> Dict[ShapelyPolygon, Tuple[int, int]]:
        ...
        for actor, vertices in actors.items():
            ...
            bbox = np.array(vertices.exterior.coords)[:-1, :]
            ...
            vec_magnitude_combination = np.tril(
                vec_magnitude * vec_magnitude.T)
            vec_magnitude_combination = np.where(
                vec_magnitude_combination == 0, self._EPSILON, vec_magnitude_combination)
            # Domain of arccos is [-1, 1]
            theta = np.tril(
                np.arccos(np.clip((vec_projection / vec_magnitude_combination), -1.0, 1.0)))
            mask = np.full(theta.shape, -np.inf)
            mask[np.tril_indices(theta.shape[0])
                 ] = theta[np.tril_indices(theta.shape[0])]
            np.fill_diagonal(mask, -np.inf)
            theta_normalized = np.where(np.logical_and(
                mask < 0, mask != -np.inf), mask % (2 * np.pi), mask)
            idx = np.unravel_index(
                np.argmax(theta_normalized), theta_normalized.shape)
            # Item containing indices of extrimities - (idx1, idx2)
            extrimities[actor] = idx
        return extrimities
    ...
    def _get_visibility_region(self, coverage_polygon: ShapelyPolygon, pivot_point: np.ndarray, actors: Dict[ShapelyPolygon, ShapelyPolygon], extrimities: Dict[ShapelyPolygon, Tuple[int, int]]) -> Dict[ShapelyPolygon, Tuple[List[np.ndarray], List[np.ndarray]]]:
        ...
        def _get_line_polygon_intersection(point: np.ndarray) -> np.ndarray:
            ...
            """
            logger.debug(
                f"{self.__LOG_PREFIX__}: Getting extreme line-polygon intersection for the sensor/actor.")
            ...
            _, u_vec = self._get_angle_and_unit_vector(
                pivot_point, np.array([point.x, point.y]))
            ...
            intersection = set_precision(extended_line, self._DEFAULT_SHAPELY_PRECISION).intersection(
                set_precision(coverage_polygon, self._DEFAULT_SHAPELY_PRECISION))
            intersection_point = None
            if intersection.is_empty:
                logger.warning(
                    f"{self.__LOG_PREFIX__}: No intersection found for the line-polygon. Actor found in the environment but was unable to compute the intersection.")
            else:
                if isinstance(intersection, ShapelyMultiPoint) or isinstance(intersection, ShapelyMultiLineString):
                    intersection_point = np.array([
                        ...
                        # (n, 1, 2) -> (n, 2) because on shapely convention
                    ]).reshape(-1, 2)
                else:
                    intersection_point = np.array(
                        intersection.coords)  # (n, 2)
                intersection_point = intersection_point[~(np.linalg.norm(
                    intersection_point - pivot_point, axis=1) < self._DISTANCE_THRESHOLD)]
                if intersection_point.size == 0:
                    ...
                    if intersection_point.shape[0] > 1:
                        ...
                        intersection_point = intersection_point[np.argmax(
                            np.linalg.norm(intersection_point - pivot_point, axis=1))]
            if intersection_point is None:
                logger.warning(
                    f"{self.__LOG_PREFIX__}: Intersection point is None. Setting it to the reference point.")
                intersection_point = np.array(point.coords)
        ...
        def _get_actor_bbox_contribution(bbox: np.ndarray, extrimities_idx: Tuple[int, int]) -> List[np.ndarray]:
            ...
            """
            logger.debug(
                f"{self.__LOG_PREFIX__}: Getting actor bbox contribution for the occluded region.")
            actor_polygon = ShapelyPolygon(bbox.tolist())
            visible_polygon = ShapelyPolygon(
                bbox[extrimities_idx, :].tolist() + [pivot_point.tolist()])
            difference = actor_polygon.difference(
                visible_polygon, grid_size=self._DEFAULT_GRID_SIZE)
            if difference.is_empty:
                difference = list(bbox[extrimities_idx, :])
            elif isinstance(difference, ShapelyPolygon):
                # Last point is the same as the first point
                difference = list(np.array(difference.exterior.coords)[:-1, :])
            else:
                difference = list(np.array(difference.coords))
            return difference

        logger.info(
            f"{self.__LOG_PREFIX__}: Getting visibility region for the sensor.")
        visibility_dict = {}
        for actor, bbox in actors.items():
            ...
            lower_bound_intersection = _get_line_polygon_intersection(
                lower_bound)
            upper_bound_intersection = _get_line_polygon_intersection(
                upper_bound)
            coverage_polygon_bounds = np.array(coverage_polygon.exterior.coords)[
                :-1, :]  # Last point is the same as the first point
            included_bounds = self._get_polygon_bounds_between_angle(coverage_polygon_bounds, np.mean(
                bbox, axis=0), upper_bound_intersection, lower_bound_intersection)
            actor_bbox_boundary = _get_actor_bbox_contribution(
                bbox, extrimities[actor])
    ...
    def _get_consolidated_occluded_region(self, visibility_dict: Dict[ShapelyPolygon, Tuple[List[np.ndarray], List[np.ndarray]]]) -> List[ShapelyPolygon]:
        ...
        """
        logger.info(
            f"{self.__LOG_PREFIX__}: Getting consolidated occluded region.")
        occluded_region = []
    ...
    def _get_occluded_region(self, pivot_point: np.ndarray, sensor_coverage: np.ndarray, timestep: int) -> Tuple[Dict[ShapelyPolygon, ShapelyPolygon], List[ShapelyPolygon], List[str]]:
        ...
        """
        logger.info(
            f"{self.__LOG_PREFIX__}: Getting occluded region for the sensor.")
        sensor_coverage_polygon = ShapelyPolygon(sensor_coverage)
        # Get all actors in the sensor coverage
        actors, actor_ids = self._get_actors_in_sensor_coverage(
            sensor_coverage_polygon, timestep)
        ...
        visibility_dict = self._get_visibility_region(
            sensor_coverage_polygon, pivot_point, actors, actors_extrimities)
        # Put all the information together
        occluded_polygons = self._get_consolidated_occluded_region(
            visibility_dict)
    ...
    def _plot_coverage(self, ax: plt.Axes, pivot_point: np.ndarray, upper_bound_point: np.ndarray, lower_bound_point: np.ndarray, timestep: int) -> Tuple[List[np.ndarray], List[ShapelyPolygon], List[str]]:
        ...
        polygon_vertices = self._get_sensor_coverage(
            ax, pivot_point, upper_bound_point, lower_bound_point)
        polygon = Polygon(polygon_vertices, edgecolor=self._AV_CAMERA_COVERAGE_COLOR,
                          facecolor=self._AV_CAMERA_COVERAGE_COLOR, alpha=self._AV_CAMERA_COVERAGE_ALPHA)
        ax.add_patch(polygon)
        actors, occluded_region_polygons, actor_ids = self._get_occluded_region(
            pivot_point, polygon_vertices, timestep)
        for occluded_region_polygon in occluded_region_polygons:
            polygon = Polygon(np.array(occluded_region_polygon.exterior.coords), edgecolor=self._OCCLUDED_BOUNDARY_EDGE_COLOR,
                              facecolor=self._OCCLUDED_BOUNDARY_FACE_COLOR, alpha=self._OCCLUDED_BOUNDARY_ALPHA)
            ax.add_patch(polygon)
    ...
    def _cast_rays(self, ax: plt.Axes, actor_heading: float, sensor_list: List[Union[InternalCameraMountResponse]], timestep: int) -> List[SensorInformation]:
        ...
        for sensor in sensor_list:
            if sensor.type == CAMERA_TYPE_NAME:
                ...
                # Get coverage bounds
                lower_coverage_bound_point = self._get_line_from_point_and_angle(
                    ax, cp, lower_fov_bound)
                upper_coverage_bound_point = self._get_line_from_point_and_angle(
                    ax, cp, upper_fov_bound)
                # Plot coverage
                actors, polygon_vertices, occluded_region_polygons, actor_ids = self._plot_coverage(
                    ax, cp, upper_coverage_bound_point, lower_coverage_bound_point, timestep)
                ...
                sensor_information.append(
                    SensorInformation(
                        ...
                        occluded_regions=[np.array(
                            occluded_region.exterior.coords) for occluded_region in occluded_region_polygons]
                    )
                )
    ...
    def _highlight_visible_actors(self, sensor_information: List[SensorInformation] = []) -> None:
        ...
        if sensor_information:
            for sensor_info in sensor_information:
                occluded_ply = list(
                    map(ShapelyPolygon, sensor_info.occluded_regions))
                combined_polygon = set_precision(unary_union(
                    occluded_ply), self._DEFAULT_SHAPELY_PRECISION)
                for actor_id, (_, resultant_actor_bbox) in sensor_info.actor_bbox_collection.items():
                    if not ShapelyPolygon(resultant_actor_bbox).within(combined_polygon):
                        ...
                        plt.scatter(mean[0], mean[1], color=self._DEFAULT_ACTOR_VISIBLE_HIGHLIGHT_COLOR,
                                    s=self._DEFAULT_ACTOR_VISIBLE_HIGHLIGHT_S, alpha=self._DEFAULT_ACTOR_VISIBLE_HIGHLIGHT_ALPHA, zorder=np.inf)
                        plt.text(mean[0] + self._DEFAULT_FONT_X_OFFSET, mean[1] + self._DEFAULT_FONT_Y_OFFSET,
                                 actor_id, fontsize=self._DEFAULT_FONT_SIZE, color=self._DEFAULT_FONT_COLOR, zorder=np.inf)
    ...
    def _generate_ground_truth_data(self, timestep: int, actor_information: List[ActorInformation], sensor_information: List[SensorInformation], av_information: AV2Vehicle, visible_bounds: ShapelyPolygon) -> AV2MotionForecastingTimeStep:
        ...
        """
        logger.info(
            f"{self.__LOG_PREFIX__}: Generating ground truth data for the timestep {timestep}.")
        for sensor_info in sensor_information:
            for actor_id, (_, resultant_actor_bbox) in sensor_info.actor_bbox_collection.items():
    ...
    def _plot_actors(self, ax: plt.Axes, timestep: int) -> AV2MotionForecastingTimeStep:
        ...
        for track in self.scenario.tracks:
            # Get actor states
            actor_positions, actor_headings, actor_velocity = self._get_actor_states(
                track, timestep)
            ...
            actor_path_color = self._TRACK_COLORS.get(
            ...
            if bbox is not None:
                ...
                if track.track_id == self._AV_ID and self.plot_occlusions:
                    av_bbox = bbox_bounds
                    sensor_information = self._plot_occlusion_map(
                        ax, bbox_bounds, actor_headings[-1], timestep)
            else:
                plt.plot(
                    actor_positions[-1][0],
                    actor_positions[-1][1],
            ...
            # Plot actor path
            av2_plot_polylines([actor_positions], style=self._DEFAULT_ACTOR_PATH_STYLE,
                               linewidth=self._DEFAULT_ACTOR_PATH_LINEWIDTH, alpha=self._DEFAULT_ACTOR_PATH_ALPHA, color=actor_path_color)
        # Highlight visible actors
        ...
        return self._generate_ground_truth_data(
            ...
            ShapelyPolygon([(xmin, ymin), (xmax, ymin),
                           (xmax, ymax), (xmin, ymax)])
        )
    ...
    def _generate_indetail_scenario_video(self, output_filename: str) -> str:
        ...
        _ = write_video(frames, video_file_path,
    ...
    def _get_analytics(self, output_filename: str, step_size: int = 1000, process_k: int = -1) -> Path:
        """
        Core implementation for getting AV2 forecasting data analytics.
        Args:
            output_filename (str): Output filename.
            step_size: Step size for which the data will be stored.
            process_k: Process k scenarios, -1 for all.
        Returns:
            Path: Path to the generated analytics file.
        """
        ...
        def get_distance_stats(distance_dict: DefaultDict[str, float]) -> Tuple[float, float, str, float, str, float]:
            ...
            """
            logger.debug(
                f"{self.__LOG_PREFIX__}: Getting actor-dictionary statistics.")
            actors, distances = list(distance_dict.keys()), list(
                distance_dict.values())
            if len(distances) == 0:
        ...
        def df_concat(df_candidate: pd.DataFrame) -> None:
            ...
            nonlocal df
            logger.debug(
                f"{self.__LOG_PREFIX__}: Concatenating the candidate dataframe with the existing dataframe.")
            if df.empty:
        ...
        logger.info(
            f"{self.__LOG_PREFIX__}: Getting analytics of the motion forecasting data.")
        ...
        if os.path.exists(csv_file_path):
            ...
            df = pd.DataFrame(
                columns=[item.value for item in AV2ForecastingAnalyticsAttributes])
        batch = []
        files = list(Path(self.input_directory).glob("*"))
        total_items = len(files)
        if process_k != -1 and process_k > 0:
            files = files[:process_k]
            total_items = process_k
        ...
        for _, item in enumerate(rich_track(files, total=total_items, description="Generating...")):
            ...
            pedestrian_dict, cyclist_dict, motorcyclist_dict, vehicle_dict, bus_dict = defaultdict(
                float), defaultdict(float), defaultdict(float), defaultdict(float), defaultdict(float)
            for timestep in range(self._OBSERVATION_DURATION_TIMESTEPS + self._PREDICTION_DURATION_TIMESTEPS):
                for track in scenario.tracks:
                    actor_positions, _, actor_velocities = self._get_actor_states(
                        track, timestep)
                    if actor_positions.shape[0] == 0 or actor_velocities.shape[0] == 0:
                        continue
                    total_distance, _ = self._compute_total_distance(actor_positions)
                    if track.track_id == self._AV_ID:
                        av_distance = total_distance
                    elif track.object_type == ObjectType.PEDESTRIAN:
                        pedestrians.add(track.track_id)
                        pedestrian_dict[track.track_id] = max(
                            pedestrian_dict[track.track_id], total_distance)
                    elif track.object_type == ObjectType.CYCLIST:
                        cyclists.add(track.track_id)
                        cyclist_dict[track.track_id] = max(
                            cyclist_dict[track.track_id], total_distance)
                    elif track.object_type == ObjectType.MOTORCYCLIST:
                        motorcyclists.add(track.track_id)
                        motorcyclist_dict[track.track_id] = max(
                            motorcyclist_dict[track.track_id], total_distance)
                    elif track.object_type == ObjectType.VEHICLE:
                        vehicles.add(track.track_id)
                        vehicle_dict[track.track_id] = max(
                            vehicle_dict[track.track_id], total_distance)
                    elif track.object_type == ObjectType.BUS:
                        buses.add(track.track_id)
                        bus_dict[track.track_id] = max(
                            bus_dict[track.track_id], total_distance)
            n_pedestrians, n_cyclists, n_motorcyclists, n_vehicles, n_buses = len(
                pedestrians), len(cyclists), len(motorcyclists), len(vehicles), len(buses)
            mean_pedestrian_distance, median_pedestrian_distance, min_pedestrian_distance_id, min_pedestrian_distance, max_pedestrian_distance_id, max_pedestrian_distance = get_distance_stats(
                pedestrian_dict)
            mean_cyclist_distance, median_cyclist_distance, min_cyclist_distance_id, min_cyclist_distance, max_cyclist_distance_id, max_cyclist_distance = get_distance_stats(
                cyclist_dict)
            mean_motorcyclist_distance, median_motorcyclist_distance, min_motorcyclist_distance_id, min_motorcyclist_distance, max_motorcyclist_distance_id, max_motorcyclist_distance = get_distance_stats(
                motorcyclist_dict)
            mean_vehicle_distance, median_vehicle_distance, min_vehicle_distance_id, min_vehicle_distance, max_vehicle_distance_id, max_vehicle_distance = get_distance_stats(
                vehicle_dict)
            mean_bus_distance, median_bus_distance, min_bus_distance_id, min_bus_distance, max_bus_distance_id, max_bus_distance = get_distance_stats(
                bus_dict)
            batch.append(
                {
        ...
        if df[AV2ForecastingAnalyticsAttributes.SCENARIO_ID.value].nunique() != len(df[AV2ForecastingAnalyticsAttributes.SCENARIO_ID.value]):
            logger.warning(
                f"{self.__LOG_PREFIX__}: There are duplicate scenario ids in the analytics file.")
    ...
    def visualize(self) -> str:
        ...
        """
        logger.info(
            f"{self.__LOG_PREFIX__}: Visualizing motion forecasting data.")
        self._init_visualization()
        if self.output_filename is None:
            self.output_filename = f"{self.scenario_id}.mp4"
        self._make_output_directory()
        if self.raw:
            return str(self._generate_scenario_video(output_filename=self.output_filename))
        return str(self._generate_indetail_scenario_video(output_filename=self.output_filename))
    
    def get_analytics(self, step_size: int = 1000, process_k: int = -1) -> str:
        """
        ...
        self._make_output_directory()
        return str(self._get_analytics(output_filename=self.output_filename, step_size=step_size, process_k=process_k))

    @classmethod
    def query_max_occurrence(cls, csv_file: str, object_type: str, top_k: int = 5, save: bool = False) -> dict:
        """
        ...
        Returns:
            dict: Dictionary containing the queried data, columns, and the path to the saved CSV file.
        """
        ...
        data = res_display.head(top_k).to_dict(orient="records")
        res = {
            "data": data,
            "columns": list(res_display.columns),
            "csv_file": csv_res_file
        }
        return res


@shared_task(name="viz.av2_motion_forecasting_dataset_single_file")
def viz_av2_motion_forecasting_dataset_single_file(*args, **kwargs):
    """Visualize AV2 Motion Forecasting Dataset - Single File."""
    return AV2Forecasting(*args, **kwargs).visualize()

@shared_task(name="viz.av2_motion_forecasting_dataset_generate_analytics")
def viz_av2_motion_forecasting_dataset_generate_analytics(*args, **kwargs):
    """Generate analytics for AV2 Motion Forecasting Dataset."""
    process_k = kwargs.get("process_k", -1)
    return AV2Forecasting(*args, **kwargs).get_analytics(process_k=process_k)

@shared_task(name="viz.av2_motion_forecasting_dataset_query_max_occurrence")
def viz_av2_motion_forecasting_dataset_query_max_occurrence(*args, **kwargs):
    """Query the maximum occurrence of the object type in the given CSV file."""
    return AV2Forecasting.query_max_occurrence(*args, **kwargs)
